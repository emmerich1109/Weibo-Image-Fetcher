...
def weibo_image_download(url, save_folder="images"):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        for idx, media in enumerate(media_urls):  # Track index here
            media_url = media['url']
            media_type = media['media_type']
            media_id = media['media_id']
            # Add zero-padded index to filename
            if 'pic' == media_type:
                save_name = os.path.join(save_folder, f"{idx:03d}_{media_id}.jpg")
            elif 'video' == media_type:
                save_name = os.path.join(save_folder, f"{idx:03d}_{media_id}.mp4")
            elif 'livephoto' == media_type:
                save_name = os.path.join(save_folder, f"{idx:03d}_{media_id}.mov")

            future = executor.submit(download_media, media_url, save_name, user_info['uid'])
            futures.append(future)

        # Wait for all futures but no need to process results in order
        for future in futures:
            future.result()  # This ensures exceptions are raised but doesn't change order
